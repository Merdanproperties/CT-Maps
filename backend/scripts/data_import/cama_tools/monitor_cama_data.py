...
from scripts.data_import.verification.cama_verification import (
    check_completeness
)
...
def generate_daily_report(db: Session) -> Dict:
    ...
    # Save report to file
    report_file = Path(__file__).parent.parent.parent.parent / "logs" / f"cama_monitoring_{datetime.now().strftime('%Y%m%d')}.txt"
